...
class Duplicates {
  ...
  /*
   * note: 1 <= a[i] <= n (n = size of array), so each a[i] - 1 is going to be an
   * indes value.
   * Loop thro the array and mark the corresponding index value as negative, the
   * repetitive number(index) will then have a -ve value
   */
  ...
  public List<Integer> findAllDuplicates(int[] nums) {
    ...
    for (int i = 0; i < nums.length; i++) {
      ...
      if (nums[index] > 0) {
        ...
        result.add(index + 1); // +1 to compensate previous step in finding index
    }
